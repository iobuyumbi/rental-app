...
const createWorkerTask = asyncHandler(async (req, res) => {
  const { orderId, taskType, workers, taskAmount, notes } = req.body;
...
const getWorkerEarnings = asyncHandler(async (req, res) => {
  ...
  const tasksWithEarnings = workerTasks.map(task => {
    ...
    const shareAmount = presentWorkers > 0 ? task.taskAmount / presentWorkers : 0;
  ...
  const totalEarnings = tasksWithEarnings.reduce((total, task) => total + task.shareAmount, 0);
