...
const OrderWorkflowButtons = ({ 
  ...
  const handleTaskSubmit = async (taskData) => {
    try {
      // The task data will be submitted through the parent component
      if (onTaskCreated) {
        await onTaskCreated({
          ...taskData,
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-md font-medium text-gray-800">Pickup & Delivery Phase</h4>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
            {taskTypes.filter(task => task.phase === 'pickup').map((taskType) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-xs font-medium text-center leading-tight">{taskType.label}</span>
        ...
        <div className="space-y-3">
          <div className="flex items-center space-x-2">
            ...
            <h4 className="text-md font-medium text-gray-800">Return & Storage Phase</h4>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
            {taskTypes.filter(task => task.phase === 'return').map((taskType) => {
              const IconComponent = taskType.icon;
              const suggestedAmount = calculateSuggestedAmount(taskType);
        ...
        <div className="text-xs text-gray-500 bg-gray-50 p-3 rounded">
          <strong>Complete Order Lifecycle:</strong> Track worker participation from initial pickup preparation 
          through final storage. Each task calculates fair remuneration based on items and effort required.
          Lunch allowances are automatically generated for participating workers.
        </div>
      </div>
